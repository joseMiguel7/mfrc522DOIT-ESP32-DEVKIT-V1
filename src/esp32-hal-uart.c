...
#include "esp32-hal-matrix.h" //JMJ
#include "esp32-hal-cpu.h" // JMJ
#include "esp32-hal-uart.h" // JMJ
...
{
    if(uart == NULL || rxPin >= SOC_GPIO_PIN_COUNT) {
        return;
    }
    pinMode(rxPin, INPUT);
...
{
    if(uart == NULL || txPin >= SOC_GPIO_PIN_COUNT) {
        return;
    }
